...
var _white_list = {
    ...
    'hao123.com': ['hao123img.com'],
    'rg.ru': ['yandex.ru', 'mail.ru', 'yandex.st', 'ria.ru', 'googleapis.com', 'google.com'],
    'kooora.com': ['googletagservices.com', 'google.com', 'googleadservices.com', 'googleapis.com', 'googlesyndication.com'],
    'mirtesen.ru':['googleapis.com', 'mtml.ru', ],
    'terra.com.br':['trrsf.com', 'terra.com'],
};
// here are some special requirements for downloading web pages for some sites.
var _settings = {
    'elwatannews.com': {
        'noreferer': true,
        'cookie': {
        ...
        'wait_js_time': 20000,
        'wait_for_time': 25000,
        'acceptable_error_codes': [301],
    },
    'alarabiya.net': {
        'noreferer': true,
        'cookie': {
            'name': 'YPF8827340282Jdskjhfiw_928937459182JAX666',
            'value': '123.126.24.14',
    ...
    'trud.ru': {
        'cookie': {
            ...
            'value': '399d93b772d805dba30a2afdba8d26cc',
            'domain': 'www.trud.com',
            'path': '/',
            'httponly': true,
        },
...
function onResourceError(resourceError) {
    ...
    if (resourceError.url == url){
        var domain = get_root_domain(resourceError.url);
        var settings = _settings[domain];
        // do not log error if site it's acceptable erorr codes
        if (settings != undefined && settings.acceptable_error_codes != null &&
                settings.acceptable_error_codes.indexOf(resourceError.errorCode) != -1){
            return;
        }
        //console.log(error_header);
...
function download(url){
    ...
    var domain = get_root_domain(url);
    var settings = _settings[domain] || {};
    if (referer != null && (settings.noreferer == undefined || !settings.noreferer)){
        page.customHeaders = {
            'Referer': referer
        };
    }
    var wait_js_time = settings.wait_js_time || 2000;
    var wait_for_time = settings.wait_for_time || 10000;
    if (settings.cookie != undefined){
        page.addCookie(settings.cookie);
    }
    // download page
